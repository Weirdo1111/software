...
export default async function ReviewPage({
  ...
  searchParams: Promise<{ lang?: string; prompt?: string }>;
}) {
  const resolvedSearchParams = await searchParams;
  const locale = await getLocale(Promise.resolve(resolvedSearchParams));
  const showHeader = !resolvedSearchParams.prompt;
