...
import type { SpeakingDifficulty, SpeakingPrompt } from "@/types/learning";
...
export function SpeakingPartnerPanel({
  ...
  return (
    <div className="rounded-[1.45rem] border border-[rgba(20,50,75,0.12)] bg-white p-4">
...
>>>>>>> Stashed changes
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--ink-soft)]">
        <span className="rounded-full bg-[rgba(20,50,75,0.05)] px-3 py-1.5">{prompt.title}</span>
        <span className="rounded-full bg-[rgba(20,50,75,0.05)] px-3 py-1.5">{prompt.partner_role}</span>
        <span className="rounded-full bg-[rgba(20,50,75,0.05)] px-3 py-1.5">{prompt.category_label}</span>
      </div>

      <div className="mt-3 grid min-h-44 max-h-56 gap-3 overflow-y-auto rounded-[1.1rem] border border-[rgba(20,50,75,0.1)] bg-[#f7f8fa] p-3">
        {partnerMessages.length > 0 ? (
          partnerMessages.map((message, index) => (
            ...
            >
              <div
                className={`max-w-[78%] rounded-[1.1rem] px-3 py-2.5 text-sm leading-6 whitespace-pre-line shadow-sm ${
                  message.role === "user"
...
>>>>>>> Stashed changes
      ...
      <label className="mt-4 grid gap-2 text-sm font-medium text-[var(--ink)]">
        ...
        <textarea
          ...
          placeholder="Respond to the task or the latest follow-up."
          className="min-h-20 rounded-[1.1rem] border border-[rgba(20,50,75,0.16)] bg-white px-4 py-3 text-sm leading-7 outline-none"
        />
      </label>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3 rounded-full border border-[rgba(20,50,75,0.12)] bg-white px-3 py-2">
          ...
          <div className="h-2.5 w-24 overflow-hidden rounded-full bg-[rgba(20,50,75,0.08)]">
            <div
      ...
      {partnerTurn.trim().length > 0 && partnerTurn.trim().length < 6 ? (
        ...
        <p className="text-sm text-[var(--ink-soft)]">Press Enter to send.</p>
      )}
